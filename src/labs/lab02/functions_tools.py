...
#     """Inject the current OpenTelemetry context as W3C traceparent headers.

#     `HTTPXClientInstrumentor` only reliably patches `httpx.Client`/`AsyncClient`
#     instances — the top-level `httpx.post` convenience function can slip past
#     it, which means the Function App starts a fresh `operation_Id` and the
#     end-to-end transaction view in App Insights breaks. Injecting the
#     propagator headers by hand makes the propagation independent of the
#     instrumentation surface. No-ops if OTel is not configured (the inject
#     call simply writes nothing).
#     """
...
#     if key:
#         params["code"] = key
#     r = httpx.post(url, json=body, params=params, headers=_trace_headers(), timeout=30)
#     r.raise_for_status()
#     return r.json()
